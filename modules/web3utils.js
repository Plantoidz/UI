...
async function loadBlockchainData(provider) {
    try {
        ...
        document.getElementById('current-balance').textContent = `${balanceDisplay} ETH`;
...
async function loadUserSeeds() {
    ...
    if (isWalletConnected && signer) {
        try {
            ...
            const contract = new ethers.Contract(networkConfig.plantoidAddress, plantoidABI, provider);
...
async function connectWallet() {
    ...
    try {
        ...
        console.log(`Wallet connected to network: ${network.chainId}, target: ${targetNetwork.chainId}`);
        ...
        isWalletConnected = true;
        console.log("✅ Wallet connection successful, isWalletConnected:", isWalletConnected);
