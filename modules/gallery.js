...
async function queryMainSubgraph(plantoidAddress) {
    try {
        ...
        console.log(`📊 Main subgraph: ${allSeeds.length} total seeds, ${filteredSeeds.length} for plantoid ${plantoidAddress}`);
...
async function queryMetadataSubgraph(plantoidAddress) {
    try {
        ...
        const metadataSubgraphs = {
            sepolia: "https://gateway-arbitrum.network.thegraph.com/api/5aa71d6a9735426594a4f8c82de56afc/subgraphs/id/EmnBAZcJGouYxmcApwMKspGqNTY79f5tw5oDh7AvqFue",
            mainnet: "https://gateway-arbitrum.network.thegraph.com/api/5aa71d6a9735426594a4f8c82de56afc/subgraphs/id/EmnBAZcJGouYxmcApwMKspGqNTY79f5tw5oDh7AvqFue"
        };
        ...
        console.log(`🔑 Metadata subgraph: found ${seedMetadatas.length} signatures`);
...
async function initializeGallery() {
    try {
        ...
        contract = new ethers.Contract(networkConfig.plantoidAddress, plantoidABI, provider);
...
function updateNetworkIndicator() {
    ...
    if (networkIndicator) {
        ...
        networkIndicator.className = `network-indicator ${networkConfig.isTestnet ? 'testnet' : 'mainnet'}`;
    }
}
...
async function loadNFTs() {
    ...
    try {
        ...
        const { maxConcurrent, delayBetweenBatches, batchSize, retryDelay, randomDelay } = RATE_LIMIT_CONFIG;
                ...
                const loadNFT = async (tokenId) => {
                    try {
                        ...
                        if (error.code === 'BAD_DATA' || error.message?.includes('Too Many Requests')) {
                            console.log(`Rate limited on NFT #${tokenId}, will retry...`);
                            ...
                            try {
                                ...
                                console.error(`Failed to load NFT #${tokenId} after retry:`, retryError.message);
                            }
                        } else {
...
async function displayNFT(tokenId, tokenURI) {
    ...
    try {
        ...
        if (animationUrl.startsWith('ipfs://')) {
            animationUrl = 'https://ipfs.io/ipfs/' + animationUrl.replace('ipfs://', '');
        }
        ...
        const isUnrevealed = !animationUrl || 
        ...
        if (isUnrevealed) {
            ...
            console.log(`Skipping NFT #${tokenId}: Unrevealed seed`);
            return;
        }
        ...
        try {
            ...
            const networkConfig = getCurrentNetworkConfig();
            const openSeaUrl = getOpenSeaUrl(networkConfig.plantoidAddress, tokenId, networkConfig.isTestnet);
            ...
            console.log(`Skipping NFT #${tokenId}: ${videoError.message}`);
            return;
        }
        
    } catch (error) {
        // Skip NFTs that fail to load metadata
        console.log(`Skipping NFT #${tokenId}: Failed to load metadata - ${error.message}`);
        return;
    }
}
...
async function loadUnrevealedNFTs() {
    ...
    try {
        ...
        document.getElementById('total-seeds').textContent = mainSeeds.length.toString();
        ...
        const combinedData = mainSeeds.map(seed => {
            ...
            console.log(`🔗 Available metadata IDs:`, metadataSignatures.map(md => md.id));
        ...
        for (const seed of combinedData) {
            const hasSignature = seed.revealedSignature && seed.revealedSignature !== '0x';
            ...
            if (notRevealed && hasSignature) {
                ...
                try {
                    const currentUri = await contract.tokenURI(seed.tokenId);
                    const isActuallyRevealed = currentUri === seed.revealedUri;
                    
                    console.log(`🔍 Seed ${seed.tokenId}: subgraph revealed=${seed.revealed}, hasSignature=${hasSignature}, actuallyRevealed=${isActuallyRevealed}`);
                    
                    if (!isActuallyRevealed) {
                        unrevealedWithSignatures.push(seed);
                    } else {
                        console.log(`⚠️ Seed ${seed.tokenId} is already revealed on-chain but subgraph shows unrevealed`);
                    }
                } catch (error) {
                    console.log(`⚠️ Could not check on-chain status for seed ${seed.tokenId}:`, error.message);
                    // If we can't check, include it anyway
                    unrevealedWithSignatures.push(seed);
                }
            } else {
                console.log(`🔍 Seed ${seed.tokenId}: revealed=${seed.revealed}, hasSignature=${hasSignature}, revealedSignature=${seed.revealedSignature}`);
            }
        }
        ...
        if (unrevealedWithSignatures.length > 0) {
            console.log(`✅ Displaying ${unrevealedWithSignatures.length} unrevealed NFTs`);
            displayUnrevealedTable(unrevealedWithSignatures);
        ...
        unrevealedLoading.style.display = 'none';
        unrevealedTable.innerHTML = '<div class="no-unrevealed">Error loading unrevealed NFTs</div>';
    }
}

// Display unrevealed NFTs in table format
function displayUnrevealedTable(unrevealedNFTs) {
    const unrevealedTable = document.getElementById('unrevealed-table');
    
    console.log('🎨 Displaying table for NFTs:', unrevealedNFTs);
    ...
    unrevealedNFTs.forEach(nft => {
        ...
        const shortAddress = holderAddress.length > 10 ? 
            `${holderAddress.slice(0, 6)}...${holderAddress.slice(-4)}` : 
            holderAddress;
            
        const tokenId = nft.tokenId || nft.id;
        const canReveal = !!nft.revealedSignature && nft.revealedSignature !== '0x' && !nft.revealed;
        
        console.log(`🎨 Rendering row for seed ${tokenId}:`, { holderAddress, canReveal, revealedSignature: nft.revealedSignature });
        
        tableHTML += `
            <div class="reveal-table-row">
                <div class="reveal-seed">Seed #${tokenId}</div>
                <div class="reveal-holder" title="${holderAddress}">${shortAddress}</div>
                <div>
...
window.revealNFT = async function(tokenId, revealedUri, revealedSignature) {
    try {
        ...
        console.log("🎭 Attempting to reveal NFT:", { tokenId, revealedUri, revealedSignature });
        ...
        try {
            const currentUri = await plantoidContract.tokenURI(tokenId);
        ...
        try {
            const gasEstimate = await plantoidContract.revealContent.estimateGas(tokenId, revealedUri, revealedSignature);
            ...
            if (gasError.data === "0xa89ac151") {
                alert("Reveal failed: Invalid signature or this NFT cannot be revealed yet.");
            } else {
        ...
        const tx = await plantoidContract.revealContent(tokenId, revealedUri, revealedSignature);
        ...
        alert("NFT revealed successfully!");
        ...
        if (error.code === "CALL_EXCEPTION") {
            alert("Transaction failed: The smart contract rejected this reveal. This could mean the signature is invalid, expired, or the NFT is already revealed.");
