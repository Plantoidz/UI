...
// Fetch IPFS metadata for a single seed
async function fetchSeedMetadata(seed) {
  if (!seed.uri) return null;
...
// Load all NFTs progressively: render a placeholder for each seed in sorted
// order, then fill each one in as its IPFS metadata arrives. The user sees
// content immediately and pieces fill in over time instead of waiting for
// every fetch to complete.
async function loadNFTs() {
  ...
  try {
    ...
    loadingMessage.textContent = "Looking up revealed seeds…";
    ...
    if (revealedSeeds.length === 0) {
      loadingMessage.textContent = "No revealed NFTs found";
      return;
    }

    // Sort descending so the placeholder grid is in order before fetches start
    revealedSeeds.sort((a, b) => parseInt(b.tokenId) - parseInt(a.tokenId));

    // Update the global status, then drop placeholder cards in immediately
    loadingMessage.textContent = `Loading ${revealedSeeds.length} seeds — videos will appear as they load…`;
    ...
    revealedSeeds.forEach((seed) => {
      ...
      nftLink.className = "nft-card nft-card-loading";
      nftLink.innerHTML = `
        <div class="nft-image-container">
          <div class="nft-skeleton" aria-hidden="true"></div>
        </div>
        <div class="nft-info">
          <div class="nft-seed-number">Seed #${seed.tokenId}</div>
        </div>
      `;
      gallery.appendChild(nftLink);
      placeholders.set(seed.id, nftLink);
    });

    let resolvedCount = 0;
    let renderedCount = 0;

    // Kick off every metadata fetch in parallel; fill each card as it lands
    const tasks = revealedSeeds.map(async (seed) => {
      const card = placeholders.get(seed.id);
      try {
        const metadata = await fetchSeedMetadata(seed);
        if (metadata && metadata.animation_url) {
          const animationUrl = ipfsToHttp(metadata.animation_url);
          const imageUrl = ipfsToHttp(metadata.image);
          card.classList.remove("nft-card-loading");
          card.querySelector(".nft-image-container").innerHTML = `
          ...
          // Revealed but no video — drop the placeholder
          card.remove();
        }
      } catch (error) {
        console.error(
        ...
        resolvedCount++;
        if (resolvedCount < revealedSeeds.length) {
          loadingMessage.textContent =
            `Loaded ${renderedCount} of ${revealedSeeds.length} seeds…`;
        }
      }
    });
...
// Initialize on page load (await resolver first so NETWORKS is populated)
(async () => {
